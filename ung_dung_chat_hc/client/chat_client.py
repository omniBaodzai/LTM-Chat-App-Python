...
class ChatClient(ttk.Frame):
    ...
    def process_incoming_data(self, data):
        ...
        if data.startswith("PUBLIC_ROOMS|"):
            rooms_str = data[len("PUBLIC_ROOMS|"):]
            self.public_rooms = [r for r in rooms_str.split(',') if r]
            print(f"Parsed PUBLIC_ROOMS: {self.public_rooms}")
            if self.update_online_users_callback:
                self.update_online_users_callback(self.public_rooms, is_rooms=True)

        elif data.startswith("ALL_USERS|"):
            users_str = data[len("ALL_USERS|"):]
            self.all_users = [u for u in users_str.split(',') if u]
            print(f"Parsed ALL_USERS: {self.all_users}")
            if self.update_online_users_callback:
                self.update_online_users_callback(self.all_users, is_users=True)

        elif data.startswith("ONLINE_USERS|"):
