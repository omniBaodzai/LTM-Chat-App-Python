...
def authenticate_user(username, hashed_password):
    ...
    try:
        ...
        print(f"[DB ERROR] Lỗi xác thực người dùng: {e}")
        return None, None
    finally:
        if conn:
            conn.close()
...
def register_user(username, hashed_password, email):
    ...
    try:
        ...
        print(f"[DB ERROR] Lỗi đăng ký người dùng: {e}")
        return False, str(e)
    finally:
        if conn:
            conn.close()
...
def create_room_in_db(room_name, user_id):
    ...
    try:
        ...
        print(f"[DB ERROR] Lỗi tạo phòng: {e}")
        return False, str(e)
    finally:
        if conn:
            conn.close()
...
def get_public_rooms_from_db():
    """Fetches list of public rooms from the database."""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        print(f"[DB ERROR] Lỗi lấy danh sách phòng: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def get_all_users_from_db():
    """Fetches list of all registered users from the database."""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        print(f"[DB ERROR] Lỗi lấy danh sách người dùng: {e}")
        return []
    finally:
        if conn:
            conn.close()
